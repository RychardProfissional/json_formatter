...
export default async function BlogPostPage({ params, searchParams }: { params: ParamsInput; searchParams: Promise<{ lang?: string }> }) {
  const { slug } = await resolveParams(params);
  const post = getBlogPost(slug);
  if (!post) notFound();
...
function renderPost(slug: string, qs: string): ReactNode {
  switch (slug) {
    case "como-formatar-json-online":
      return (
        <>
          ...
          <P>
            Para um fluxo rápido no dia a dia: <Link className="font-semibold text-blue-700 hover:underline dark:text-blue-300" href={`/tools/json/formatter${qs}`}>abrir o Formatador de JSON</Link>.
          </P>
        </>
      );
    ...
    case "ferramentas-uteis-para-programadores":
      return (
        <>
          ...
          <P>
            <Link className="font-semibold text-blue-700 hover:underline dark:text-blue-300" href={`/tools${qs}`}>
    ...
    case "como-validar-cpf-json-regex":
      return (
        <>
          ...
          <P>
            <Link className="font-semibold text-blue-700 hover:underline dark:text-blue-300" href={`/tools/json/validator${qs}`}>
