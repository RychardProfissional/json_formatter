...
export function JsonMinifyClient() {
  ...
  const [status, setStatus] = useState<string>(() =>
    t("tools.json.minify.status.idle")
  );
  ...
  return (
    <ToolPage>
      <ToolHeader
        title={t("tools.json.minify.title")}
        subtitle={t("tools.json.minify.subtitle")}
      />
      ...
      <ToolSection title={t("tools.json.minify.section.when")}>
        <p className="text-slate-600 dark:text-slate-300">
          {t("tools.json.minify.when.body")}
        </p>
      </ToolSection>

      <ToolSection title={t("tools.json.minify.section.cautions")}>
