...
export async function generateMetadata({ searchParams }: { searchParams: Promise<{ lang?: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `/tools/pdf${qs}` },
    openGraph: {
      type: "website",
      title: dict["tools.pdf.index.meta.ogTitle"],
      description: dict["tools.pdf.index.meta.description"],
      url: `${SITE.url}/tools/pdf${qs}`,
      images: ["/og.svg"]
    },
    twitter: {
...
export default async function PdfToolsIndexPage({ searchParams }: { searchParams: Promise<{ lang?: string }> }) {
  const sp = await searchParams;
  const lang = sp?.lang;
  const dict = getDict(lang);
  const qs = localeQuery(lang);
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10">
      ...
      <p className="mt-10 text-sm text-slate-500 dark:text-slate-400">
        ...
        <Link className="font-semibold" href={`/tools${qs}`}>
