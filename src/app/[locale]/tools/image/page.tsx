...
export function generateMetadata({ params }: { params: Params }): Metadata {
  ...
  return {
    ...
    alternates: { canonical: `${p}/tools/image` },
    openGraph: {
      type: "website",
      url: `${SITE.url}${p}/tools/image`,
      title: dict["tools.image.index.meta.title"],
      description: dict["tools.image.index.meta.description"],
...
export default function ToolsImagePage({ params }: { params: Params }) {
  const dict = getDict(params.locale);
  const p = localePrefix(params.locale);
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      ...
      <p className="mt-10 text-sm text-slate-500 dark:text-slate-400">
        ...
        <Link className="font-semibold" href={`${p}/tools`}>{dict["tools.image.index.backTo.link"]}</Link>
