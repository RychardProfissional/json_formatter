...
export function generateMetadata({ params }: { params: Params }): Metadata {
  ...
  return {
    ...
    alternates: { canonical: `${p}/tools` },
    openGraph: {
      type: "website",
      title: `${dict["tools.index.meta.title"]} | Respawn Tech`,
      description: dict["tools.index.meta.description"],
      url: `${SITE.url}${p}/tools`,
      images: ["/og.svg"]
    },
    twitter: {
...
export default function ToolsIndexPage({ params }: { params: Params }) {
  const dict = getDict(params.locale);
  const p = localePrefix(params.locale);
  ...
  const tools = [
    ...
    { href: `${p}/tools/image/compressor`, name: dict["tools.image.index.cards.compressor.title"], desc: dict["tools.image.index.cards.compressor.desc"] },
    { href: `${p}/tools/pdf`, name: dict["tools.index.cards.pdf.title"], desc: dict["tools.index.cards.pdf.desc"] }
  ];
    ...
    <main className="mx-auto max-w-5xl px-4 py-10">
      ...
      <div className="mt-6 space-y-3 text-slate-600 dark:text-slate-300">
        ...
        <p>
          ...
          <Link className="font-semibold" href={`${p}/tools/json/formatter`}>
            {dict["tools.index.intro.p2.link1"]}
          </Link>
          {dict["tools.index.intro.p2.between"]}
          <Link className="font-semibold" href={`${p}/tools/json/validator`}>
      ...
      <section className="mt-10 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        ...
        <p className="mt-2 text-slate-600 dark:text-slate-300">
          ...
          <Link className="font-semibold" href={`${p}/politica-de-privacidade`}>
      ...
      <p className="mt-10 text-sm text-slate-500 dark:text-slate-400">
        ...
        <Link className="font-semibold" href={`${p}/`}>{dict["tools.index.backHome.link"]}</Link>
